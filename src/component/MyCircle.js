var vynch = (function() {
	...
	function getRoomsList(callback)
	{
		...
		req.open('GET', 'https://www.mycircle.tv/srv/userooms.php?id=28-89949-542', true);
	...
	function innerSetRoomsList()
	{
		...
		for ( var ir = 0 ; ir < vynch.rooms.length ; ir++ ) {
			...
			(room.pl_size == 0 ? '': '<li>' + room.pl_size + '&nbsp;videos in the playlist</li>') +
			(room.users == 0 ? '': '<li class="vynch_users">' + room.users + '&nbsp;Attendees</li>') +
			'</ul><div style="visibility:hidden;padding-bottom:5px;">&nbsp;</div><div class="vynch_incit">' +
			'<a href="javascript:vynch.openRoom(\'' + room.id + '\')">_en_vynch_open_room</a></div>' +
			'</td>';
		}
		txt += '</tr></table>';				
	...
	function openRoom(rid)
	{
		...
		if ( vynch_div ) {
			vynch_div.innerHTML = '<iframe id="vynch_frame" style="width:' + 800 + 'px;height:' + 580 + 
				'px;border-width:1px;border-style:solid;border-color:#EEEEEE;" src="https://www.mycircle.tv/srv/frame.php?r=' + 
				rid + '&tabs=yes"></iframe>';
...
// vynch.openRoom('159-6058-404');

export function showDiv(ref,id){
    const source_div = document.createElement('div');
    source_div.id = "vynch_div";
    source_div.style = '{{display:inline-block}}'
    ref.current.appendChild(source_div);
    vynch.openRoom(id);
}